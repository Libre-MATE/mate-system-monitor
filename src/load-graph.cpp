...
static void draw_background(LoadGraph *graph) {
  ...
  graph->graph_dely =
      (graph->draw_height - 15) / num_bars; /* round to int to avoid AA blur */
  graph->real_draw_height = graph->graph_dely * num_bars;
  graph->graph_delx =
      (graph->draw_width - 2.0 - graph->rmargin - graph->indent) /
      (LoadGraph::NUM_POINTS - 3);
  graph->graph_buffer_offset = (int)(1.5 * graph->graph_delx) + FRAME_WIDTH;

  gtk_widget_get_allocation(graph->disp, &allocation);
  surface = gdk_window_create_similar_surface(
      gtk_widget_get_window(graph->disp), CAIRO_CONTENT_COLOR_ALPHA,
      allocation.width, allocation.height);
  cr = cairo_create(surface);

  GtkStyleContext *context =
      gtk_widget_get_style_context(ProcData::get_instance()->notebook);
  gtk_style_context_save(context);
  gtk_style_context_set_state(context, GTK_STATE_FLAG_NORMAL);
  gtk_style_context_get_background_color(
      context, gtk_style_context_get_state(context), &bg);
  ...
  pango_font_description_set_size(font_desc,
                                  0.8 * graph->fontsize * PANGO_SCALE);
  ...
  for (i = 0; i <= num_bars; ++i) {
    ...
    if (graph->type == LOAD_GRAPH_NET) {
      ...
      caption = g_format_size_full(
          network_in_bits ? rate * 8 : rate,
    ...
    if (i == 0 || i == num_bars)
      ...
      cairo_set_source_rgba(cr, 0.89, 0.89, 0.89, 1.0);
    cairo_move_to(cr, graph->rmargin + graph->indent - 3,
  ...
  const unsigned total_seconds =
      graph->speed * (LoadGraph::NUM_POINTS - 2) / 1000;
  ...
  for (unsigned int i = 0; i < 7; i++) {
    ...
    cairo_line_to(cr, (ceil(x) + 0.5) + graph->rmargin + graph->indent,
    ...
    pango_layout_get_extents(layout, NULL, &extents);
    cairo_move_to(cr,
                  ((ceil(x) + 0.5) + graph->rmargin + graph->indent) -
                      (1.0 * extents.width / PANGO_SCALE / 2),
...
void load_graph_queue_draw(LoadGraph *graph) {
  /* repaint */
  gtk_widget_queue_draw(graph->disp);
}

static int load_graph_update(
    gpointer user_data);  // predeclare load_graph_update so we can compile ;)

static gboolean load_graph_configure(GtkWidget *widget,
...
static gboolean load_graph_draw(GtkWidget *widget, cairo_t *context,
                                gpointer data_ptr) {
  LoadGraph *const graph = static_cast<LoadGraph *>(data_ptr);
  GdkWindow *window;
  ...
  /* Subframe offset */
  x_offset += graph->rmargin -
              ((sample_width / graph->frames_per_unit) * graph->render_counter);
  ...
  cairo_rectangle(cr, graph->rmargin + graph->indent + FRAME_WIDTH + 1,
                  FRAME_WIDTH - 1,
  ...
  for (j = 0; j < graph->n; ++j) {
    cairo_move_to(cr, x_offset,
                  (1.0f - graph->data[0][j]) * graph->real_draw_height);
    gdk_cairo_set_source_rgba(cr, &(graph->colors[j]));

    for (i = 1; i < LoadGraph::NUM_POINTS; ++i) {
      if (graph->data[i][j] == -1.0f) continue;
      cairo_curve_to(
          cr, x_offset - ((i - 0.5f) * graph->graph_delx),
...
static void get_load(LoadGraph *graph) {
  guint i;
  glibtop_cpu cpu;
...
void set_memory_label_and_picker(GtkLabel *label, GSMColorButton *picker,
  ...
  if (total == 0) {
    ...
    text = g_strdup_printf(_("%s (%.1f%%) of %s"), used_text, 100.0 * percent,
                           total_text);
  }
  ...
  if (picker) gsm_color_button_set_fraction(picker, percent);
}
}  // namespace

static void get_memory(LoadGraph *graph) {
...
static void net_scale(LoadGraph *graph, guint64 din, guint64 dout) {
  ...
  if (ProcData::get_instance()->config.network_in_bits) {
    ...
    procman_debug("bak*8 %" G_GUINT64_FORMAT
                  ", ticks %d, d %f"
    ...
    guint64 coef10 =
        std::ceil(new_max / double(G_GUINT64_CONSTANT(1) << (base10 * 10)));
  ...
  if ((0.8 * graph->net.max) < new_max && new_max <= graph->net.max) return;
  ...
  procman_debug("rescale dmax = %" G_GUINT64_FORMAT " max = %" G_GUINT64_FORMAT
...
static void get_net(LoadGraph *graph) {
  ...
  for (i = 0; i < netlist.number; ++i) {
    ...
    if (netload.if_flags & (1 << GLIBTOP_IF_FLAGS_LOOPBACK)) continue;
    ...
    /* Skip interfaces without any IPv4/IPv6 address (or
     ...
     possible too.  */
    if (not(netload.flags & (1 << GLIBTOP_NETLOAD_ADDRESS6) and
            netload.scope6 != GLIBTOP_IF_IN6_SCOPE_LINK) and
        not(netload.flags & (1 << GLIBTOP_NETLOAD_ADDRESS)))
      continue;
...
static gboolean load_graph_update(gpointer user_data) {
  ...
  if (graph->draw) load_graph_queue_draw(graph);
...
LoadGraph::~LoadGraph() {
  ...
  if (timer_index) g_source_remove(timer_index);

  clear_background();
}

static gboolean load_graph_destroy(GtkWidget *widget, gpointer data_ptr) {
  LoadGraph *const graph = static_cast<LoadGraph *>(data_ptr);
...
LoadGraph::LoadGraph(guint type)
  ...
  // FIXME:
  // on configure, graph->frames_per_unit =
  // graph->draw_width/(LoadGraph::NUM_POINTS); knock FRAMES down to 5 until
  // cairo gets faster
  ...
  switch (type) {
    ...
    case LOAD_GRAPH_MEM:
      ...
      mem_color_picker = gsm_color_button_new(&colors[0], GSMCP_TYPE_PIE);
      swap_color_picker = gsm_color_button_new(&colors[1], GSMCP_TYPE_PIE);
      break;
    case LOAD_GRAPH_NET:
...
void load_graph_start(LoadGraph *graph) {
...
void load_graph_stop(LoadGraph *graph) {
  /* don't draw anymore, but continue to poll */
  graph->draw = FALSE;
}

void load_graph_change_speed(LoadGraph *graph, guint new_speed) {
  if (graph->speed == new_speed) return;
...
LoadGraphLabels *load_graph_get_labels(LoadGraph *graph) {
  return &graph->labels;
}

GtkWidget *load_graph_get_widget(LoadGraph *graph) {
  return graph->main_widget;
}

GtkWidget *load_graph_get_mem_color_picker(LoadGraph *graph) {
  return graph->mem_color_picker;
}

GtkWidget *load_graph_get_swap_color_picker(LoadGraph *graph) {
