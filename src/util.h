...
gboolean load_symbols(const char *module, ...) G_GNUC_NULL_TERMINATED;
...
#define procman_debug(FMT, ...) \
  procman_debug_real(__FILE__, __LINE__, __func__, FMT, ##__VA_ARGS__)
...
template <typename T>
void tree_store_update(GtkTreeModel *model, GtkTreeIter *iter, int column,
                       const T &new_value) {
  T current_value;
...
template <>
inline void tree_store_update<char>(GtkTreeModel *model, GtkTreeIter *iter,
                                    int column, char *new_value) {
