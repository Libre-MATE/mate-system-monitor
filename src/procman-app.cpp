...
static void mount_changed(const Glib::RefPtr<Gio::Mount> &) {
  cb_update_disks(ProcData::get_instance());
}

static void init_volume_monitor(ProcData *procdata) {
  using namespace Gio;
...
static gboolean has_key(gchar **keys, const gchar *key) {
  gchar **loop = keys;

  while (*loop) {
    if (!strcmp(*loop++, key)) return TRUE;
  }

  return FALSE;
}

static void tree_changed_cb(GSettings *settings, const gchar *key,
                            gpointer data) {
  ProcData *procdata = static_cast<ProcData *>(data);

  procdata->config.show_tree = g_settings_get_boolean(settings, key);

  g_object_set(G_OBJECT(procdata->tree), "show-expanders",
               procdata->config.show_tree, NULL);

  proctable_clear_tree(procdata);
  proctable_update(procdata);
}

static void solaris_mode_changed_cb(GSettings *settings, const gchar *key,
                                    gpointer data) {
  ProcData *procdata = static_cast<ProcData *>(data);
...
static void network_in_bits_changed_cb(GSettings *settings, const gchar *key,
                                       gpointer data) {
  ProcData *procdata = static_cast<ProcData *>(data);
...
static void view_as_changed_cb(GSettings *settings, const gchar *key,
                               gpointer data) {
  ProcData *procdata = static_cast<ProcData *>(data);
...
static void warning_changed_cb(GSettings *settings, const gchar *key,
                               gpointer data) {
  ProcData *procdata = static_cast<ProcData *>(data);
...
static void timeouts_changed_cb(GSettings *settings, const gchar *key,
                                gpointer data) {
  ProcData *procdata = static_cast<ProcData *>(data);
  ...
  if (g_str_equal(key, "update-interval")) {
    ...
    procdata->config.graph_update_interval =
        MAX(procdata->config.graph_update_interval, 250);
...
static void color_changed_cb(GSettings *settings, const gchar *key,
                             gpointer data) {
...
static void show_all_fs_changed_cb(GSettings *settings, const gchar *key,
                                   gpointer data) {
  ProcData *const procdata = static_cast<ProcData *>(data);
...
static ProcData *procman_data_new(GSettings *settings) {
  ...
  g_settings_get(settings, "window-state", "(iiii)", &pd->config.width,
                 &pd->config.height, &pd->config.xpos, &pd->config.ypos);
  ...
  g_signal_connect(settings, "changed::show-all-fs",
  ...
  monitor = gdk_display_get_monitor_at_point(display, pd->config.xpos,
                                             pd->config.ypos);
  if (monitor == NULL) {
  ...
  pd->config.disks_update_interval =
      MAX(pd->config.disks_update_interval, 1000);
  pd->config.whose_process = CLAMP(pd->config.whose_process, 0, 2);
  pd->config.current_tab =
      CLAMP(pd->config.current_tab, PROCMAN_TAB_SYSINFO, PROCMAN_TAB_DISKS);
...
static void procman_free_data(ProcData *procdata) {
  proctable_free_table(procdata);
  delete procdata->smooth_refresh;
}

ProcmanApp::ProcmanApp()
    : Gtk::Application("org.mate.SystemMonitor",
...
static void set_tab(GtkNotebook *notebook, gint tab, ProcData *procdata) {
  gtk_notebook_set_current_page(notebook, tab);
  cb_change_current_page(notebook, tab, procdata);
}

int ProcmanApp::on_command_line(
