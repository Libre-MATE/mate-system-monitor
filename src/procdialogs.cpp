...
static void kill_dialog_button_pressed(GtkDialog *dialog, gint id,
                                       gpointer data) {
  struct KillArgs *kargs = static_cast<KillArgs *>(data);

  gtk_widget_destroy(GTK_WIDGET(dialog));

  if (id == GTK_RESPONSE_OK) kill_process(kargs->procdata, kargs->signal);

  g_free(kargs);
}

void procdialog_create_kill_dialog(ProcData *procdata, int signal) {
  ...
  kargs->signal = signal;
  gint selected_count =
      gtk_tree_selection_count_selected_rows(procdata->selection);
  ...
  if (selected_count == 1) {
    ...
    if (signal == SIGKILL) {
      ...
      primary = g_strdup_printf(_("Are you sure you want to kill the selected "
                                  "process “%s” (PID: %u)?"),
                                selected_process->name, selected_process->pid);
    } else {
      /*xgettext: primary alert message for ending single process*/
      primary = g_strdup_printf(_("Are you sure you want to end the selected "
                                  "process “%s” (PID: %u)?"),
                                selected_process->name, selected_process->pid);
    }
  } else {
    if (signal == SIGKILL) {
      ...
      primary = g_strdup_printf(
          _("Are you sure you want to kill the %d selected processes?"),
          selected_count);
    } else {
      /*xgettext: primary alert message for ending multiple processes*/
      primary = g_strdup_printf(
          _("Are you sure you want to end the %d selected processes?"),
          selected_count);
    }
  }
  ...
  if (signal == SIGKILL) {
    ...
    secondary =
        _("Killing a process may destroy data, break the "
    ...
    /*xgettext: secondary alert message*/
    secondary =
        _("Ending a process may destroy data, break the "
          "session or introduce a security risk. "
          "Only unresponsive processes should be ended.");
    button_text = ngettext("_End Process", "_End Processes", selected_count);
  }

  kill_alert_dialog = gtk_message_dialog_new(
      GTK_WINDOW(procdata->app),
      static_cast<GtkDialogFlags>(GTK_DIALOG_MODAL |
                                  GTK_DIALOG_DESTROY_WITH_PARENT),
      GTK_MESSAGE_WARNING, GTK_BUTTONS_NONE, "%s", primary);
  g_free(primary);

  gtk_message_dialog_format_secondary_text(
      GTK_MESSAGE_DIALOG(kill_alert_dialog), "%s", secondary);

  gtk_dialog_add_buttons(GTK_DIALOG(kill_alert_dialog), "gtk-cancel",
                         GTK_RESPONSE_CANCEL, button_text, GTK_RESPONSE_OK,
                         NULL);
...
static void renice_scale_changed(GtkAdjustment *adj, gpointer data) {
  GtkWidget *label = GTK_WIDGET(data);

  new_nice_value = int(gtk_adjustment_get_value(adj));
  gchar *text = g_strdup_printf(_("(%s Priority)"),
                                procman::get_nice_level(new_nice_value));
  gtk_label_set_text(GTK_LABEL(label), text);
  g_free(text);
}

static void renice_dialog_button_pressed(GtkDialog *dialog, gint id,
                                         gpointer data) {
  ProcData *procdata = static_cast<ProcData *>(data);

  if (id == 100) {
    if (new_nice_value == -100) return;
...
void procdialog_create_renice_dialog(ProcData *procdata) {
  ...
  if (selected_count == 1) {
    dialog_title = g_strdup_printf(
        _("Change Priority of Process “%s” (PID: %u)"), info->name, info->pid);
  } else {
    dialog_title = g_strdup_printf(
        _("Change Priority of %d Selected Processes"), selected_count);
  }
  ...
  renice_adj =
      gtk_adjustment_new(info->nice, RENICE_VAL_MIN, RENICE_VAL_MAX, 1, 1, 0);
  new_nice_value = 0;
...
static void prefs_dialog_button_pressed(GtkDialog *dialog, gint id,
                                        gpointer data) {
  if (id == GTK_RESPONSE_HELP) {
    GError *error = 0;
    if (!g_app_info_launch_default_for_uri(
            "help:mate-system-monitor/mate-system-monitor-prefs", NULL,
            &error)) {
...
class SpinButtonUpdater {
 public:
  SpinButtonUpdater(const string &key) : key(key) {}

  static gboolean callback(GtkWidget *widget, GdkEventFocus *event,
                           gpointer data) {
 ...
 private:
  void update(GtkSpinButton *spin) {
    ...
    g_settings_set_int(ProcData::get_instance()->settings, this->key.c_str(),
                       new_value);
...
static void field_toggled(const gchar *child_schema,
  ...
  gboolean toggled;
  GSettings *settings =
      g_settings_get_child(ProcData::get_instance()->settings, child_schema);
  gchar *key;
  int id;
...
static void proc_field_toggled(GtkCellRendererToggle *cell, gchar *path_str,
                               gpointer data) {
  field_toggled("proctree", cell, path_str, data);
}

static void disk_field_toggled(GtkCellRendererToggle *cell, gchar *path_str,
                               gpointer data) {
  field_toggled("disktreenew", cell, path_str, data);
}

static void create_field_page(GtkBuilder *builder, GtkWidget *tree,
  ...
  for (it = columns; it; it = it->next) {
    ...
    if (!title) title = _("Icon");

    column_id = gtk_tree_view_column_get_sort_column_id(column);
    if ((column_id == COL_CGROUP) && (!cgroups_enabled())) continue;

    if ((column_id == COL_UNIT || column_id == COL_SESSION ||
         column_id == COL_SEAT || column_id == COL_OWNER)
#ifdef HAVE_SYSTEMD
        && !LOGIND_RUNNING()
#endif
    )
...
void procdialog_create_preferences_dialog(ProcData *procdata) {
  ...
  adjustment = (GtkAdjustment *)gtk_adjustment_new(
      update / 1000.0, MIN_UPDATE_INTERVAL / 1000, MAX_UPDATE_INTERVAL / 1000,
      0.25, 1.0, 0);
  ...
  g_settings_bind(procdata->settings, SmoothRefresh::KEY.c_str(), smooth_button,
                  "active", G_SETTINGS_BIND_DEFAULT);

  check_button = GET_WIDGET("check_button");
  g_settings_bind(procdata->settings, "kill-dialog", check_button, "active",
                  G_SETTINGS_BIND_DEFAULT);

  GtkWidget *solaris_button = GET_WIDGET("solaris_button");
  g_settings_bind(procdata->settings, procman::settings::solaris_mode.c_str(),
                  solaris_button, "active", G_SETTINGS_BIND_DEFAULT);

  create_field_page(builder, procdata->tree, "proctree");

  update = (gfloat)procdata->config.graph_update_interval;
  adjustment = (GtkAdjustment *)gtk_adjustment_new(update / 1000.0, 0.25, 100.0,
                                                   0.25, 1.0, 0);
  ...
  g_settings_bind(procdata->settings,
                  procman::settings::network_in_bits.c_str(), bits_button,
                  "active", G_SETTINGS_BIND_DEFAULT);

  update = (gfloat)procdata->config.disks_update_interval;
  adjustment = (GtkAdjustment *)gtk_adjustment_new(update / 1000.0, 1.0, 100.0,
                                                   1.0, 1.0, 0);
  ...
  g_settings_bind(procdata->settings, "show-all-fs", check_button, "active",
                  G_SETTINGS_BIND_DEFAULT);
...
static char *procman_action_to_command(ProcmanActionType type, gint pid,
                                       gint extra_value) {
  switch (type) {
