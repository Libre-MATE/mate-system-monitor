...
namespace procman {
void SelectionMemento::add_to_selected(GtkTreeModel* model, GtkTreePath*,
                                       GtkTreeIter* iter, gpointer data) {
  guint pid = 0;
  gtk_tree_model_get(model, iter, COL_PID, &pid, -1);
  if (pid) static_cast<SelectionMemento*>(data)->pids.push_back(pid);
}
...
void SelectionMemento::save(GtkWidget* tree) {
  ...
  gtk_tree_selection_selected_foreach(selection,
                                      &SelectionMemento::add_to_selected, this);
}
  ...
  if (not this->pids.empty()) {
    ...
    for (iterator it(this->pids.begin()); it != this->pids.end(); ++it) {
      if (ProcInfo* proc = ProcInfo::find(*it)) {
        ...
        procman_debug("Could not find process %u, cannot re-select it",
                      unsigned(*it));
    }
  }
}
