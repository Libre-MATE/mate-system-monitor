...
static void renice_single_process(GtkTreeModel *model, GtkTreePath *path,
  ...
  if (!info) return;
  if (info->nice == args->nice_value) return;
  ...
  if (errno == EPERM || errno == EACCES) {
    ...
    success = procdialog_create_root_password_dialog(
        PROCMAN_ACTION_RENICE, args->procdata, info->pid, args->nice_value);
...
static void kill_single_process(GtkTreeModel *model, GtkTreePath *path,
                                GtkTreeIter *iter, gpointer data) {
  ...
  if (errno == EPERM) {
    ...
    success = procdialog_create_root_password_dialog(
        PROCMAN_ACTION_KILL, args->procdata, info->pid, args->signal);
