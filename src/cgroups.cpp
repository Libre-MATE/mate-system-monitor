...
static void append_cgroup_name(char *line, gchar **current_cgroup_name) {
  ...
  *(path - 1) = '\0';
  ...
  if ((std::strcmp(path, "/") == 0) ||
      (std::strncmp(controller, "name=", 5) == 0))
    return;
  ...
  if ((tmp = g_strstr_len(*current_cgroup_name, -1, path_plus_space))) {
    ...
    *(tmp + off) = ')';
    ...
    tmp = g_strdup_printf("%s, %s(%s)", *current_cgroup_name, path_plus_space,
                          controller);
...
static int check_cgroup_changed(gchar *line, gchar *current_cgroup_set) {
  ...
  *(path - 1) = '\0';
  ...
  if (std::strncmp(controller, "name=", 5) == 0) goto out;
  ...
  while ((found = g_strstr_len(tmp, -1, path))) {
    ...
    close_paren = g_strstr_len(found, -1, ")");
    if (*found == ' ') {
      if (g_strstr_len(found + 1, close_paren - found, controller)) goto out;
    }
    tmp = close_paren + 1;
  }
  ret = 1;
...
void get_process_cgroup_info(ProcInfo *info) {
  ...
  if (!path) return;
  if (!g_file_get_contents(path, &text, NULL, NULL)) goto out;
  ...
  if (cgroups_changed) {
    ...
    if (info->cgroup_name) g_free(info->cgroup_name);
    if (!cgroup_name)
