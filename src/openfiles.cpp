...
enum { COL_FD, COL_TYPE, COL_OBJECT, COL_OPENFILE_STRUCT, NUM_OPENFILES_COL };

static const char *get_type_name(enum glibtop_file_type t) {
  switch (t) {
...
static char *friendlier_hostname(const char *addr_str, int port) {
  ...
  if (getaddrinfo(addr_str, port_str, &hints, &res)) goto failsafe;

  if (getnameinfo(res->ai_addr, res->ai_addrlen, hostname, sizeof hostname,
                  service, sizeof service, NI_IDN))
    goto failsafe;
...
static void add_new_files(gpointer key, gpointer value, gpointer data) {
  glibtop_open_files_entry *openfiles =
      static_cast<glibtop_open_files_entry *>(value);
  ...
  gtk_list_store_set(
      GTK_LIST_STORE(model), &row, COL_FD, openfiles->fd, COL_TYPE,
...
static gboolean classify_openfiles(GtkTreeModel *model, GtkTreePath *path,
  ...
  openfiles = static_cast<glibtop_open_files_entry *>(
      g_hash_table_lookup(new_maps, old_name));
  if (openfiles) {
...
static gboolean compare_open_files(gconstpointer a, gconstpointer b) {
  const glibtop_open_files_entry *o1 =
      static_cast<const glibtop_open_files_entry *>(a);
  const glibtop_open_files_entry *o2 =
      static_cast<const glibtop_open_files_entry *>(b);

  /* Falta manejar los diferentes tipos! */
  return (o1->fd == o2->fd) && (o1->type == o2->type); /* XXX! */
}

static void update_openfiles_dialog(GtkWidget *tree) {
  ProcInfo *info;
  GtkTreeModel *model;
  glibtop_open_files_entry *openfiles;
  ...
  pid_t pid = GPOINTER_TO_UINT(
      static_cast<pid_t *>(g_object_get_data(G_OBJECT(tree), "selected_info")));
  ...
  new_maps = static_cast<GHashTable *>(
      g_hash_table_new_full(g_str_hash, compare_open_files, NULL, NULL));
  ...
  while (old_maps) {
    ...
    gtk_tree_model_get(model, iter, COL_OPENFILE_STRUCT, &openfiles, -1);
...
static void close_openfiles_dialog(GtkDialog *dialog, gint id, gpointer data) {
  GtkWidget *tree = static_cast<GtkWidget *>(data);
  GSettings *settings;
  guint timer;

  settings =
      static_cast<GSettings *>(g_object_get_data(G_OBJECT(tree), "settings"));
  procman_save_tree_state(settings, tree,
                          procman::settings::open_files_tree_prefix.c_str());
...
static GtkWidget *create_openfiles_tree(ProcData *procdata) {
  ...
  const gchar *const titles[] = {
      ...
      N_("FD"), N_("Type"), N_("Object")};

  model = gtk_list_store_new(NUM_OPENFILES_COL, G_TYPE_INT, /* FD */
                             G_TYPE_STRING,                 /* Type */
  ...
  for (i = 0; i < NUM_OPENFILES_COL - 1; i++) {
    ...
    column = gtk_tree_view_column_new_with_attributes(_(titles[i]), cell,
                                                      "text", i, NULL);
  ...
  procman_get_tree_state(procdata->settings, tree,
                         procman::settings::open_files_tree_prefix.c_str());

  return tree;
}

static gboolean openfiles_timer(gpointer data) {
  GtkWidget *tree = static_cast<GtkWidget *>(data);
  GtkTreeModel *model;
...
static void create_single_openfiles_dialog(GtkTreeModel *model,
  ...
  openfilesdialog = gtk_dialog_new_with_buttons(
      _("Open Files"), NULL, GTK_DIALOG_DESTROY_WITH_PARENT, "gtk-close",
  ...
  label = procman_make_label_for_mmaps_or_ofiles(
      _("_Files opened by process \"%s\" (PID %u):"), info->name, info->pid);
  ...
  g_object_set_data(G_OBJECT(tree), "selected_info",
                    GUINT_TO_POINTER(info->pid));
...
void create_openfiles_dialog(ProcData *procdata) {
  gtk_tree_selection_selected_foreach(procdata->selection,
                                      create_single_openfiles_dialog, procdata);
}
