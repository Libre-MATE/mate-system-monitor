...
static gboolean load_selinux(void) {
  return load_symbols("libselinux.so.1", "getpidcon", &getpidcon, "freecon",
                      &freecon, "is_selinux_enabled", &is_selinux_enabled,
                      NULL);
}

void get_process_selinux_context(ProcInfo *info) {
  char *con;
...
gboolean can_show_security_context_column(void) {
  if (!(has_selinux = load_selinux())) return FALSE;
