...
void SmoothRefresh::status_changed(GSettings *settings, const gchar *key,
                                   gpointer user_data) {
...
void SmoothRefresh::load_settings_value(const gchar *key) {
  ...
  if (this->active) procman_debug("smooth_refresh is enabled");
}
...
bool SmoothRefresh::get(guint &new_interval) {
  const unsigned config_interval =
      ProcData::get_instance()->config.update_interval;
  ...
  /*
    invariant: MAX_UPDATE_INTERVAL >= interval >= config_interval >=
    MIN_UPDATE_INTERVAL
  ...
  */
  ...
  else if (this->interval != config_interval && pcpu < PCPU_LO &&
           this->last_pcpu < PCPU_LO)
  ...
  if (changed) this->interval = new_interval;

  this->last_pcpu = pcpu;

  if (changed) {
    procman_debug(
        "CPU usage is %3u%%, changed refresh_interval to %u (config %u)",
        this->last_pcpu, this->interval, config_interval);
  }
