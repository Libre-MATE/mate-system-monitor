 ...
 public:
  ...
  virtual void set_distro_labels(GtkWidget *name, GtkWidget *release) {
    ...
    if (this->distro_release != "") {
      ...
      markup = g_strdup_printf(_("Release %s %s"), this->distro_release.c_str(),
                               SysInfo::get_os_type().c_str());
    } else {
      /* Translators: The string parameter is the architecture, 32 or 64-bit */
      markup = g_strdup_printf(_("%s Version"), SysInfo::get_os_type().c_str());
    }
#else
    /* Translators: The string parameter is release version (codename) */
    markup = g_strdup_printf(_("Release %s"), this->distro_release.c_str());
#endif

    g_object_set(G_OBJECT(release), "label", markup, NULL);

    g_free(markup);
  }

  static string release() { return uname().release; }

  static string system() { return uname().sysname; }
 ...
 private:
  ...
  static char *prettify_info(const char *info) {
    ...
    for (uint i = 0; i < G_N_ELEMENTS(rs); i++) {
      ...
      result = g_regex_replace_literal(re, pretty, -1, 0, rs[i].replacement,
                                       mflags, &error);
  ...
  static char *get_renderer_from_helper(gboolean discrete_gpu) {
    ...
    if (!g_spawn_sync(NULL, (char **)argv, envp, G_SPAWN_DEFAULT, NULL, NULL,
                      &renderer, NULL, &status, &error)) {
      g_debug("Failed to get %s GPU: %s",
              discrete_gpu ? "discrete" : "integrated", error->message);
      return NULL;
    }

    if (!g_spawn_check_exit_status(status, NULL)) return NULL;

    if (renderer == NULL || *renderer == '\0') return NULL;
  ...
  void load_graphics_info() {
    ...
    try {
      ...
      if (error != NULL) {
        throw std::runtime_error(
            "Unable to connect to create a proxy for org.gnome.SessionManager");
      }

      renderer_variant =
          g_dbus_proxy_get_cached_property(session_proxy, "Renderer");
      if (!renderer_variant) {
        throw std::runtime_error(
            "Unable to retrieve org.gnome.SessionManager.Renderer property.");
      }
    ...
    if (renderer == NULL) renderer = get_renderer_from_helper(FALSE);

    if (renderer == NULL) renderer = get_renderer_from_helper(TRUE);
  ...
  void load_processors_info() {
    ...
    for (uint i = 0; i != info->ncpu; ++i) {
      ...
      for (int j = 0; model == NULL && j != G_N_ELEMENTS(keys); ++j) {
        model = static_cast<char *>(
            g_hash_table_lookup(info->cpuinfo[i].values, keys[j]));
      }
      ...
      if (count == NULL)
        ...
        g_hash_table_replace(counts, model,
  ...
  void load_disk_info() {
    ...
    for (guint i = 0; i != mountlist.number; ++i) {
      ...
      if (string(entries[i].devname).find("/dev/") != 0) continue;

      if (string(entries[i].mountdir).find("/media/") == 0) continue;
  ...
  void load_uname_info() {
    ...
    this->kernel =
        string(uname().sysname) + ' ' + uname().release + ' ' + uname().machine;
  ...
  void load_mate_version() {
    ...
    if (not(context = xmlXPathNewContext(document))) return;
...
class FreeBSDSysInfo : public SysInfo {
 public:
  FreeBSDSysInfo() { this->load_freebsd_info(); }
...
class SolarisSysInfo : public SysInfo {
 public:
  SolarisSysInfo() { this->load_solaris_info(); }
...
class LSBSysInfo : public SysInfo {
 public:
  LSBSysInfo() : re(Glib::Regex::create("^.+?:\\s*(.+)\\s*$")) {
    // start();
  }
 ...
 private:
  ...
  std::istream &get_value(std::istream &is, string &s) const {
    if (std::getline(is, s)) this->strip_description(s);
    return is;
  }
  ...
  void read_lsb(Glib::Pid pid, int status) {
    ...
    Glib::RefPtr<Glib::IOChannel> channel =
        Glib::IOChannel::create_from_fd(this->lsb_fd);
    ...
    this->get_value(input, this->distro_name) and
        this->get_value(input, release) and this->get_value(input, codename);
  ...
  void start() {
    ...
    sigc::slot<void, GPid, int> slot =
        sigc::mem_fun(this, &LSBSysInfo::read_lsb);
  ...
  void sync_lsb_release() {
    ...
    if (g_spawn_command_line_sync("lsb_release -irc", &out, 0, &status,
                                  &error)) {
      string release, codename;
      if (!error and WIFEXITED(status) and WEXITSTATUS(status) == 0) {
        std::istringstream input(out);
        this->get_value(input, this->distro_name) and
            this->get_value(input, release) and
            this->get_value(input, codename);
        this->distro_release = release;
        if (codename != "" && codename != "n/a")
          this->distro_release += " (" + codename + ')';
      }
...
class NetBSDSysInfo : public SysInfo {
 public:
  NetBSDSysInfo() { this->load_netbsd_info(); }
 ...
 private:
  void load_netbsd_info() {
    ...
    if (input) std::getline(input, this->distro_release);
  }
};

class GenericSysInfo : public SysInfo {
 public:
  GenericSysInfo() { this->load_os_release(); }
...
class MintSysInfo : public SysInfo {
 public:
  MintSysInfo() { this->load_os_release(); }
...
class OpenBSDSysInfo : public SysInfo {
 public:
  OpenBSDSysInfo() { this->load_openbsd_info(); }
 ...
 private:
  void load_openbsd_info() {
    ...
    if (input) std::getline(input, this->kernel);
  }
};
...
static gboolean sysinfo_logo_draw(GtkWidget *widget, cairo_t *context,
                                  gpointer data_ptr) {
  ...
  cairo_arc(cr, X_PAD + LOGO_W - RADIUS, Y_PAD + RADIUS, RADIUS, -0.5 * M_PI,
            0);
  cairo_line_to(cr, X_PAD + LOGO_W, Y_PAD + LOGO_H - RADIUS);
  cairo_arc(cr, X_PAD + LOGO_W - RADIUS, Y_PAD + LOGO_H - RADIUS, RADIUS, 0,
            0.5 * M_PI);
  cairo_line_to(cr, X_PAD + RADIUS, Y_PAD + LOGO_H);
  cairo_arc(cr, X_PAD + RADIUS, Y_PAD + LOGO_H - RADIUS, RADIUS, 0.5 * M_PI,
            -1.0 * M_PI);
  cairo_line_to(cr, X_PAD, Y_PAD + RADIUS);
  cairo_arc(cr, X_PAD + RADIUS, Y_PAD + RADIUS, RADIUS, -1.0 * M_PI,
            -0.5 * M_PI);

  cp = cairo_pattern_create_linear(0, Y_PAD, 0, Y_PAD + LOGO_H);
  style = gtk_widget_get_style(widget);
  cairo_pattern_add_color_stop_rgba(
      cp, 0.0, style->base[GTK_STATE_SELECTED].red / 65535.0,
      style->base[GTK_STATE_SELECTED].green / 65535.0,
      style->base[GTK_STATE_SELECTED].blue / 65535.0, 1.0);
  cairo_pattern_add_color_stop_rgba(
      cp, 1.0, style->base[GTK_STATE_SELECTED].red / 65535.0,
      style->base[GTK_STATE_SELECTED].green / 65535.0,
      style->base[GTK_STATE_SELECTED].blue / 65535.0, 0.0);
...
static GtkWidget *add_section(GtkBox *vbox, const char *title, int num_row,
                              int num_col, GtkWidget **out_frame) {
  GtkWidget *table;
...
static GtkWidget *add_row(GtkGrid *table, const char *label, const char *value,
                          int row) {
  ...
  gtk_grid_attach(table, header, 0, row, 1, 1);
  ...
  gtk_grid_attach(table, label_widget, 1, row, 1, 1);

  return label_widget;
}

static GtkWidget *procman_create_sysinfo_view(void) {
  GtkWidget *hbox;
  GtkWidget *vbox;
  ...
  /* left-side logo */
  if (g_file_test(DATADIR "/pixmaps/mate-system-monitor/distribution/side.png",
                  G_FILE_TEST_EXISTS)) {
    logo = gtk_image_new_from_file(
        DATADIR "/pixmaps/mate-system-monitor/distribution/side.png");
  } else {
  ...
  gtk_label_set_use_markup(
      GTK_LABEL(gtk_frame_get_label_widget(GTK_FRAME(hostname_frame))), TRUE);
  ...
  if (data->mate_version != "") table_size++;
  distro_table =
  ...
  gtk_grid_attach(GTK_GRID(distro_table), distro_release_label, 0, table_count,
                  1, 1);
  table_count++;
  data->set_distro_labels(gtk_frame_get_label_widget(GTK_FRAME(distro_frame)),
  ...
  g_free(markup);
  gtk_label_set_xalign(GTK_LABEL(header), 0.0);
  gtk_grid_attach(GTK_GRID(distro_table), header, 0, table_count, 1, 1);
  table_count++;
  ...
  add_row(GTK_GRID(hardware_table), _("Processor:"), data->processors.c_str(),
          1);
  add_row(GTK_GRID(hardware_table), _("Graphics:"), data->graphics.c_str(), 2);
...
void build_sysinfo_ui() {
  ...
  if (!ui) {
    ...
    GtkBox *box = GTK_BOX(gtk_notebook_get_nth_page(
        GTK_NOTEBOOK(procdata->notebook), PROCMAN_TAB_SYSINFO));
