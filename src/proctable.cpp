...
static void cb_columns_changed(GtkTreeView *treeview, gpointer user_data) {
  ProcData *const procdata = static_cast<ProcData *>(user_data);

  procman_save_tree_state(procdata->settings, GTK_WIDGET(treeview), "proctree");
}

static void cb_sort_changed(GtkTreeSortable *model, gpointer user_data) {
  ProcData *const procdata = static_cast<ProcData *>(user_data);

  procman_save_tree_state(procdata->settings, GTK_WIDGET(procdata->tree),
                          "proctree");
}

static GtkTreeViewColumn *my_gtk_tree_view_get_column_with_sort_column_id(
    GtkTreeView *treeview, int id) {
  GList *columns, *it;
  ...
  for (it = columns; it; it = it->next) {
    if (gtk_tree_view_column_get_sort_column_id(
            static_cast<GtkTreeViewColumn *>(it->data)) == id) {
...
GSList *proctable_get_columns_order(GtkTreeView *treeview) {
  ...
  for (col = columns; col; col = col->next) {
    ...
    id = gtk_tree_view_column_get_sort_column_id(
        static_cast<GtkTreeViewColumn *>(col->data));
...
static void cb_proctable_column_resized(GtkWidget *widget, GParamSpec *pspec,
                                        gpointer data) {
...
static gboolean search_equal_func(GtkTreeModel *model, gint column,
  ...
  gtk_tree_model_get(model, iter, COL_NAME, &name, COL_USER, &user, -1);
...
GtkWidget *proctable_new(ProcData *const procdata) {
  GtkWidget *proctree;
  GtkTreeStore *model;
  ...
  const gchar *titles[] = {
      N_("Process Name"), N_("User"), N_("Status"), N_("Virtual Memory"),
      N_("Resident Memory"), N_("Writable Memory"), N_("Shared Memory"),
      N_("X Server Memory"),
      /* xgettext:no-c-format */ N_("% CPU"), N_("CPU Time"), N_("Started"),
      N_("Nice"), N_("ID"), N_("Security Context"), N_("Command Line"),
      N_("Memory"),
      /* xgettext: combined noun, the function the process is waiting in, see
         wchan ps(1) */
      N_("Waiting Channel"), N_("Control Group"), N_("Unit"), N_("Session"),
      /* TRANSLATORS: Seat = i.e. the physical seat the session of the process
      belongs to, only for multi-seat environments. See
      http://en.wikipedia.org/wiki/Multiseat_configuration */
      N_("Seat"), N_("Owner"), N_("Disk Read Total"), N_("Disk Write Total"),
      N_("Disk Read"), N_("Disk Write"), N_("Priority"), NULL, "POINTER"};

  gint i;

  model = gtk_tree_store_new(NUM_COLUMNS, G_TYPE_STRING, /* Process Name */
                             G_TYPE_STRING,              /* User         */
                             G_TYPE_UINT,                /* Status       */
  ...
  g_object_set(G_OBJECT(proctree), "show-expanders", procdata->config.show_tree,
               NULL);
  ...
  gtk_tree_view_column_set_attributes(column, cell_renderer, "surface",
                                      COL_SURFACE, NULL);

  cell_renderer = gtk_cell_renderer_text_new();
  gtk_tree_view_column_pack_start(column, cell_renderer, FALSE);
  gtk_tree_view_column_set_attributes(column, cell_renderer, "text", COL_NAME,
                                      NULL);
  ...
  for (i = COL_USER; i <= COL_PRIORITY; i++) {
    ...
    switch (i) {
      ...
      case COL_MEMWRITABLE:
        gtk_tree_view_column_set_cell_data_func(
            col, cell, &procman::memory_size_na_cell_data_func,
            GUINT_TO_POINTER(i), NULL);
        break;

      case COL_CPU_TIME:
        gtk_tree_view_column_set_cell_data_func(
            col, cell, &procman::duration_cell_data_func, GUINT_TO_POINTER(i),
            NULL);
        break;
      ...
      case COL_DISK_WRITE_TOTAL:
        gtk_tree_view_column_set_cell_data_func(
            col, cell, &procman::storage_size_na_cell_data_func,
            GUINT_TO_POINTER(i), NULL);
        break;
      ...
      case COL_PRIORITY:
        gtk_tree_view_column_set_cell_data_func(
            col, cell, &procman::priority_cell_data_func,
  ...
  if (!can_show_security_context_column()) {
    column = my_gtk_tree_view_get_column_with_sort_column_id(
        GTK_TREE_VIEW(proctree), COL_SECURITYCONTEXT);
    gtk_tree_view_column_set_visible(column, FALSE);
  }

  if (!cgroups_enabled()) {
    column = my_gtk_tree_view_get_column_with_sort_column_id(
        GTK_TREE_VIEW(proctree), COL_CGROUP);
    gtk_tree_view_column_set_visible(column, FALSE);
  }
...
static void get_process_name(ProcInfo *info, const gchar *cmd,
                             const GStrv args) {
  if (args) {
...
std::string ProcInfo::lookup_user(guint uid) {
  ...
  // procman_debug("User lookup for uid %u: %s", uid, (p.second ? "MISS" :
  // "HIT"));
...
void ProcInfo::set_user(guint uid) {
  if (G_LIKELY(this->uid == uid)) return;
...
#else
    if (maps[i].perm & GLIBTOP_MAP_PERM_WRITE) memwritable += maps[i].size;
#endif
  }
...
static void get_process_memory_info(ProcInfo *info) {
  ...
  if (GDK_IS_X11_DISPLAY(gdk_display_get_default())) {
    ...
    wnck_pid_read_resource_usage(
        gdk_screen_get_display(gdk_screen_get_default()), info->pid,
        &xresources);
...
static void update_info_mutable_cols(ProcInfo *info) {
  ...
  tree_store_update(model, &info->node, COL_DISK_READ_TOTAL,
                    info->disk_read_bytes_total);
  tree_store_update(model, &info->node, COL_DISK_WRITE_TOTAL,
                    info->disk_write_bytes_total);
  tree_store_update(model, &info->node, COL_DISK_READ_CURRENT,
                    info->disk_read_bytes_current);
  tree_store_update(model, &info->node, COL_DISK_WRITE_CURRENT,
                    info->disk_write_bytes_current);
...
static void insert_info_to_tree(ProcInfo *info, ProcData *procdata,
                                bool forced = false) {
  GtkTreeModel *model;
  ...
  if (procdata->config.show_tree) {
    ...
    if (not forced) parent = ProcInfo::find(info->ppid);

    if (parent) {
      GtkTreePath *parent_node = gtk_tree_model_get_path(model, &parent->node);
      gtk_tree_store_insert(GTK_TREE_STORE(model), &info->node, &parent->node,
                            0);

      if (!gtk_tree_view_row_expanded(GTK_TREE_VIEW(procdata->tree),
                                      parent_node)
#ifdef __linux__
          // on linuxes we don't want to expand kthreadd by default (always has
          // pid 2)
          && (parent->pid != 2)
#endif
      )
        gtk_tree_view_expand_row(GTK_TREE_VIEW(procdata->tree), parent_node,
                                 FALSE);
  ...
  gtk_tree_store_set(GTK_TREE_STORE(model), &info->node, COL_POINTER, info,
                     COL_NAME, info->name, COL_ARGS, info->arguments,
                     COL_TOOLTIP, info->tooltip, COL_PID, info->pid,
                     COL_SECURITYCONTEXT, info->security_context, -1);
...
static void remove_info_from_tree(ProcData *procdata, GtkTreeModel *model,
                                  ProcInfo *current, List &orphans,
                                  unsigned lvl = 0) {
  ...
  if (procdata->selected_process == current) procdata->selected_process = NULL;
...
static void get_process_systemd_info(ProcInfo *info) {
#ifdef HAVE_SYSTEMD
  uid_t uid;

  if (!LOGIND_RUNNING()) return;
  ...
  if (info->session != NULL) sd_session_get_seat(info->session, &info->seat);
...
static void update_info(ProcData *procdata, ProcInfo *info) {
  ...
  if (difference > 0) info->status = GLIBTOP_PROCESS_RUNNING;
  ...
  info->disk_write_bytes_current = difference > update_interval_seconds
                                       ? difference / update_interval_seconds
                                       : 0ULL;
  difference = procio.disk_rbytes - info->disk_read_bytes_total;
  info->disk_read_bytes_current = difference > update_interval_seconds
                                      ? difference / update_interval_seconds
                                      : 0ULL;
...
ProcInfo::ProcInfo(pid_t pid)
  ...
  if (tooltip.empty()) tooltip = procstate.cmd;
  ...
  if (it != ProcInfo::cpu_times.end()) {
    if (proctime.rtime >= it->second) cpu_time = it->second;
  }
...
static void refresh_list(ProcData *procdata, const pid_t *pid_list,
                         const guint n) {
  ...
  if (procdata->config.show_tree) {
    ...
    while (not addition.empty()) {
      ...
      while (it != addition.end()) {
        procman_debug("looking for %d's parent with ppid %d", int((*it)->pid),
                      int((*it)->ppid));
        ...
        if (not parent) {
          // or std::find(addition.begin(), addition.end(), parent) ==
          // addition.end()) {
...
static void proctable_update_list(ProcData *const procdata) {
  pid_t *pid_list;
  ...
  /* FIXME: total cpu time elapsed should be calculated on an individual basis
   *here
  ...
  /* proclist.number == g_list_length(procdata->info) ==
   * g_hash_table_size(procdata->pids) */
}

void proctable_update(ProcData *const procdata) {
  char *string;
...
void proctable_free_table(ProcData *const procdata) {
  for (ProcInfo::Iterator it(ProcInfo::begin()); it != ProcInfo::end(); ++it)
    delete it->second;
...
void proctable_clear_tree(ProcData *const procdata) {
  GtkTreeModel *model;
...
void ProcInfo::set_icon(Glib::RefPtr<Gdk::Pixbuf> icon) {
  this->surface = gdk_cairo_surface_create_from_pixbuf(icon->gobj(), 0, NULL);

  GtkTreeModel *model;
  model =
      gtk_tree_view_get_model(GTK_TREE_VIEW(ProcData::get_instance()->tree));
  gtk_tree_store_set(GTK_TREE_STORE(model), &this->node, COL_SURFACE,
                     this->surface, -1);
}
