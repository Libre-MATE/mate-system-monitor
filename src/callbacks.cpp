...
void cb_kill_sigstop(GtkAction *action, gpointer data) {
...
void cb_kill_sigcont(GtkAction *action, gpointer data) {
  ProcData *const procdata = static_cast<ProcData *>(data);
...
static void kill_process_helper(ProcData *procdata, int sig) {
...
void cb_about(GtkAction *action, gpointer data) {
  ...
  const gchar *const documenters[] = {"Bill Day", "Sun Microsystems", NULL};

  const gchar *const artists[] = {"Baptiste Mille-Mathias", NULL};

  const gchar *license[] = {
      N_("System Monitor is free software; you can redistribute it and/or "
         "modify "
      ...
      N_("You should have received a copy of the GNU General Public License "
         "along with System Monitor; if not, write to the Free Software "
         "Foundation, Inc., "
  ...
  gtk_show_about_dialog(
      GTK_WINDOW(procdata->app), "program-name", _("System Monitor"), "version",
      VERSION, "title", _("About System Monitor"), "comments",
      _("View current processes and monitor system state"), "copyright",
      _("Copyright \xc2\xa9 2001-2004 Kevin Vandersloot\n"
        "Copyright \xc2\xa9 2005-2007 Benoît Dejean\n"
        "Copyright \xc2\xa9 2011-2020 MATE developers"),
      "logo-icon-name", "utilities-system-monitor", "authors", authors,
      "artists", artists, "documenters", documenters, "translator-credits",
      _("translator-credits"), "license", license_trans, "wrap-license", TRUE,
...
void cb_help_contents(GtkAction *action, gpointer data) {
  GError *error = 0;
  if (!g_app_info_launch_default_for_uri("help:mate-system-monitor", NULL,
                                         &error)) {
...
gboolean cb_app_delete(GtkWidget *window, GdkEventAny *event, gpointer data) {
  ...
  if (procdata->timeout) g_source_remove(procdata->timeout);
  if (procdata->disk_timeout) g_source_remove(procdata->disk_timeout);
...
void cb_cpu_color_changed(GSMColorButton *cp, gpointer data) {
  char key[80];
...
void cb_mem_color_changed(GSMColorButton *cp, gpointer data) {
  ProcData *const procdata = static_cast<ProcData *>(data);
  change_settings_color(procdata->settings, "mem-color", cp);
}

void cb_swap_color_changed(GSMColorButton *cp, gpointer data) {
  ProcData *const procdata = static_cast<ProcData *>(data);
  change_settings_color(procdata->settings, "swap-color", cp);
}

void cb_net_in_color_changed(GSMColorButton *cp, gpointer data) {
  ProcData *const procdata = static_cast<ProcData *>(data);
  change_settings_color(procdata->settings, "net-in-color", cp);
}

void cb_net_out_color_changed(GSMColorButton *cp, gpointer data) {
  ProcData *const procdata = static_cast<ProcData *>(data);
...
void cb_row_selected(GtkTreeSelection *selection, gpointer data) {
  ...
  */
  ...
  if (procdata->selected_process) {
    ...
    GtkRadioAction *normal = GTK_RADIO_ACTION(
        gtk_action_group_get_action(procdata->action_group, "Normal"));
...
void cb_switch_page(GtkNotebook *nb, GtkWidget *page, gint num, gpointer data) {
  cb_change_current_page(nb, num, data);
}

void cb_change_current_page(GtkNotebook *nb, gint num, gpointer data) {
  ProcData *const procdata = static_cast<ProcData *>(data);
  ...
  if (num == PROCMAN_TAB_PROCESSES) {
    ...
    if (!procdata->timeout)
      procdata->timeout =
          g_timeout_add(procdata->config.update_interval, cb_timeout, procdata);
  ...
  if (num == PROCMAN_TAB_DISKS) {
    ...
    if (!procdata->disk_timeout) {
      procdata->disk_timeout = g_timeout_add(
          procdata->config.disks_update_interval, cb_update_disks, procdata);
    }
  } else {
    if (procdata->disk_timeout) {
...
gint cb_user_refresh(GtkAction *, gpointer data) {
  ProcData *const procdata = static_cast<ProcData *>(data);
...
gint cb_timeout(gpointer data) {
  ...
  if (!procdata->terminating) {
    ...
    if (procdata->smooth_refresh->get(new_interval)) {
      procdata->timeout = g_timeout_add(new_interval, cb_timeout, procdata);
      return FALSE;
    }
  }
  return TRUE;
...
void cb_radio_processes(GtkAction *action, GtkRadioAction *current,
                        gpointer data) {
  ProcData *const procdata = static_cast<ProcData *>(data);
