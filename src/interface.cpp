...
static const GtkActionEntry menu_entries[] = {
    ...
    {"StopProcess", NULL, N_("_Stop Process"), "<control>S", N_("Stop process"),
     G_CALLBACK(cb_kill_sigstop)},
    ...
    {"MemoryMaps", NULL, N_("_Memory Maps"), "<control>M",
     N_("Open the memory maps associated with a process"),
     G_CALLBACK(cb_show_memory_maps)},
    ...
    {"ProcessProperties", NULL, N_("_Properties"), NULL,
     N_("View additional information about a process"),
     G_CALLBACK(cb_show_process_properties)},

    {"HelpContents", "help-browser", N_("_Contents"), "F1",
     N_("Open the manual"), G_CALLBACK(cb_help_contents)},
    {"About", "help-about", N_("_About"), NULL, N_("About this application"),
     G_CALLBACK(cb_about)}};
...
static const GtkRadioActionEntry priority_menu_entries[] = {
    ...
    {"High", NULL, N_("High"), NULL, N_("Set process priority to high"),
     HIGH_PRIORITY},
    {"Normal", NULL, N_("Normal"), NULL, N_("Set process priority to normal"),
     NORMAL_PRIORITY},
    {"Low", NULL, N_("Low"), NULL, N_("Set process priority to low"),
     LOW_PRIORITY},
    {"VeryLow", NULL, N_("Very Low"), NULL,
     N_("Set process priority to very low"), VERY_LOW_PRIORITY},
    {"Custom", NULL, N_("Custom"), NULL, N_("Set process priority manually"),
     CUSTOM_PRIORITY}};
...
static const char ui_info[] =
    ...
    "      <menuitem name=\"ViewActiveProcesses\" "
    "action=\"ShowActiveProcesses\" />"
    ...
    "      <separator />"
    "      <menuitem name=\"ViewProcessPropertiesMenu\" "
    "action=\"ProcessProperties\" />"
    "      <separator />"
...
static void create_proc_view(ProcData *procdata, GtkBuilder *builder) {
  GtkWidget *proctree;
  GtkWidget *scrolled;
  char *string;
  ...
  procdata->loadavg =
      GTK_WIDGET(gtk_builder_get_object(builder, "load_avg_label"));
  ...
  procdata->popup_menu =
      gtk_ui_manager_get_widget(procdata->uimanager, "/PopupMenu");
}
...
static void create_sys_view(ProcData *procdata, GtkBuilder *builder) {
  ...
  GtkWidget *flowbox =
      GTK_WIDGET(gtk_builder_get_object(builder, "cpu_flow_box"));
  ...
  gtk_box_pack_start(GTK_BOX(mem_graph_box), load_graph_get_widget(mem_graph),
                     TRUE, TRUE, 0);
  ...
  gtk_box_pack_start(GTK_BOX(net_graph_box), load_graph_get_widget(net_graph),
                     TRUE, TRUE, 0);
  ...
  picker_alignment =
      GTK_WIDGET(gtk_builder_get_object(builder, "receiving_picker_alignment"));
  ...
  picker_alignment =
      GTK_WIDGET(gtk_builder_get_object(builder, "sending_picker_alignment"));
  gtk_container_add(GTK_CONTAINER(picker_alignment), color_picker);
...
static void menu_item_deselect_cb(GtkMenuItem *proxy, ProcData *procdata) {
...
static void connect_proxy_cb(GtkUIManager *manager, GtkAction *action,
                             GtkWidget *proxy, ProcData *procdata) {
...
void create_main_window(ProcData *procdata) {
  ...
  if (visual) gtk_widget_set_visual(app, visual);
  ...
  gtk_window_add_accel_group(
      GTK_WINDOW(app), gtk_ui_manager_get_accel_group(procdata->uimanager));

  if (!gtk_ui_manager_add_ui_from_string(procdata->uimanager, ui_info, -1,
                                         NULL)) {
  ...
  gtk_action_group_add_actions(procdata->action_group, menu_entries,
                               G_N_ELEMENTS(menu_entries), procdata);
  gtk_action_group_add_toggle_actions(
      procdata->action_group, toggle_menu_entries,
      G_N_ELEMENTS(toggle_menu_entries), procdata);

  gtk_action_group_add_radio_actions(procdata->action_group, radio_menu_entries,
                                     G_N_ELEMENTS(radio_menu_entries),
                                     procdata->config.whose_process,
                                     G_CALLBACK(cb_radio_processes), procdata);

  gtk_action_group_add_radio_actions(
      procdata->action_group, priority_menu_entries,
      G_N_ELEMENTS(priority_menu_entries), NORMAL_PRIORITY,
      G_CALLBACK(cb_renice), procdata);

  gtk_ui_manager_insert_action_group(procdata->uimanager,
                                     procdata->action_group, 0);
  ...
  for (i = 0; i < 4; ++i) {
    ...
    gtk_accel_group_connect(accel_group, '0' + (i + 1), GDK_MOD1_MASK,
  ...
  action =
      gtk_action_group_get_action(procdata->action_group, "ShowDependencies");
...
void do_popup_menu(ProcData *procdata, GdkEventButton *event) {
  gtk_menu_popup_at_pointer(GTK_MENU(procdata->popup_menu), NULL);
}

void update_sensitivity(ProcData *data) {
  const char *const selected_actions[] = {
      "StopProcess",    "ContProcess", "EndProcess", "KillProcess",
      "ChangePriority", "MemoryMaps",  "OpenFiles",  "ProcessProperties"};

  const char *const processes_actions[] = {
      "ShowActiveProcesses", "ShowAllProcesses", "ShowMyProcesses",
      "ShowDependencies", "Refresh"};
  ...
  selected_sensitivity =
      (processes_sensitivity && data->selection &&
       gtk_tree_selection_count_selected_rows(data->selection) > 0);
  ...
  for (i = 0; i != G_N_ELEMENTS(processes_actions); ++i) {
    action =
        gtk_action_group_get_action(data->action_group, processes_actions[i]);
    gtk_action_set_sensitive(action, processes_sensitivity);
  }

  for (i = 0; i != G_N_ELEMENTS(selected_actions); ++i) {
    action =
        gtk_action_group_get_action(data->action_group, selected_actions[i]);
...
void block_priority_changed_handlers(ProcData *data, bool block) {
  ...
  if (block) {
    for (i = 0; i != G_N_ELEMENTS(priority_menu_entries); ++i) {
      GtkRadioAction *action = GTK_RADIO_ACTION(gtk_action_group_get_action(
          data->action_group, priority_menu_entries[i].name));
...
static void cb_toggle_tree(GtkAction *action, gpointer data) {
  ...
  if (show == procdata->config.show_tree) return;
